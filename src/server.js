...
var server = http.createServer(function(req, res) {
  ...
  if(req.url.indexOf('receive-auth-token') > -1){
    exchange.code(req.url).then(function(token){
      ...
      res.setHeader('Set-Cookie', [route.authentication['cookie-name'] +'=' + token.access_token], route.authentication['cookie-name'] + '_RT=' + token.refresh_token);
      res.statusCode = 307;
      res.end();
      return;
    }, function(error){
    ...
    validate.authentication(req, route).then(function(authentication){
      if(authentication.redirect){
        ...
        //todo: allow host override for aws
        ...
        console.log("redirect 1: " + redirectBack);
