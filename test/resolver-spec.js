...
describe('resolver', function () {
    ...
    it('returns whole cache object', function () {
        sinon.stub(cache, "getRoutes").returns([{route: 'abc/def', fruit: "banana"}]);

        assert.equal(resolver.resolveRoute("http://monsanto.com/abc/def/ghi", "").fruit, 'banana');
    });

    it('adds service urls', function () {
        sinon.stub(cache, "getRoutes").returns([{route: 'abc/def', services: ['service1']}]);

        sinon.stub(cache, "getServices").returns({service1: [{url: "www.monsanto.com"}]});
